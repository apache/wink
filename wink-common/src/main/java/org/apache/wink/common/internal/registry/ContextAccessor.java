...
//        // the map holds the methods that can be accessible using the @Context annotation
//        try {
//            contextAccessors.put(HttpServletRequest.class, RuntimeContext.class.getMethod("getHttpServletRequest"));
//            contextAccessors.put(HttpServletResponse.class, RuntimeContext.class.getMethod("getHttpServletResponse"));
//            contextAccessors.put(UriInfo.class, RuntimeContext.class.getMethod("getUriInfo"));
//            contextAccessors.put(HttpHeaders.class, RuntimeContext.class.getMethod("getHttpHeaders"));
//            contextAccessors.put(Request.class, RuntimeContext.class.getMethod("getRequest"));
//            contextAccessors.put(SecurityContext.class, RuntimeContext.class.getMethod("getSecurityContext"));
//            contextAccessors.put(Providers.class, RuntimeContext.class.getMethod("getProviders"));
//            contextAccessors.put(ServletContext.class, RuntimeContext.class.getMethod("getServletContext"));
//            contextAccessors.put(ServletConfig.class, RuntimeContext.class.getMethod("getServletConfig"));
//            contextAccessors.put(LinkBuilders.class, RuntimeContext.class.getMethod("getLinkBuilders"));
//        } catch (Exception e) {
//            // should never happen
...
//    }
    ...
    /**
     * Obtain a context from one of the contexts available through the RuntimeContext. This can be
     * done in two ways: if runtimeContext is provided, the context is obtained directly. If runtimeContext
     * is null, then a proxy is created which obtains the RuntimeContext using the TLS.
     * 
     * @param <T>
    ...
    public <T> T getContextFromAccessor(final Class<T> contextClass, RuntimeContext runtimeContext) {
        ...
        if (contextClass.isInterface() == false) {
            throw new IllegalArgumentException(String.format("%s must be an interface", contextClass));
        }
        ...
        // return a proxy that looks for a context on TLS during the method invocation
        T proxy = (T)Proxy.newProxyInstance(Injectable.class.getClassLoader(), new Class[]{contextClass},
                new InvocationHandler() {
                    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
                        ...
                        Object context = runtimeContext.getAttribute(contextClass);
    ...
    public <T> T getContextFromResolver(Class<T> contextClass, RuntimeContext runtimeContext) {
        ...
        if (providers != null) {
            ...
            ContextResolver<T> contextResolver = providers.getContextResolver(contextClass, mediaType);
