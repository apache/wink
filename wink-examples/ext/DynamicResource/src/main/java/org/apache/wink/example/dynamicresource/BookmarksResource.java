...
public class BookmarksResource extends AbstractDynamicResource {
    ...
    /**
     * This method is invoked when the HTTP GET method is issued by the client. This occurs only
     * when the requested representation (Http Accept header) is Atom (application/atom+xml) or Json
     * (application/json). The feed is created with the mandatory fields. The feed will also contain
     * the entries found in the BookmarksStore. Links are generated for the feed and for the entries
     * in the response.
     * 
     * @return the SyndFeed instance that will be serialized as an Atom or as Json
     */
    @GET
    ...
    public SyndFeed getBookmarks(@Context LinkBuilders linkBuilders, @Context UriInfo uriInfo) {
        ...
        // add entries to the feed, based on the existing bookmarks in the memory store
        // (feed entries have no content, they have just metadata so there is no need to set content
        // here)
    ...
    /**
     * This method is invoked when the HTTP POST method is issued by the client. This occurs only
     * when the requested representation (Http Accept header) is Atom (application/atom+xml) or Json
     * (application/json) and plain text is provided in the HTTP request message body of content
     * MIME type "text/plain" (header "Content-Type" must be "text/plain"). This method creates a
     * new Bookmark resource based on the data in the request and puts the new Bookmark into the
     * BookmarkStore.
     * 
    ...
    @Produces({MediaType.APPLICATION_ATOM_XML, MediaType.APPLICATION_JSON})
    public Response createBookmark(String bookmark, @Context UriInfo uriInfo, @Context LinkBuilders linkBuilders) {
        ...
        SystemLinksBuilder entryLinksBuilder = linkBuilders.createSystemLinksBuilder().subResource(bookmarkId);
     ...
     * This method is invoked when the HTTP GET method is issued by the client. This occurs only
     * when the requested representation (Http Accept header) is Atom (application/atom+xml) or Json
     * (application/json). In the case that the requested bookmark is found in the BookmarkStore a
     * synd entry is created with mandatory metadata fields and with metadata content that is taken
     * from the BookmarkStore.
     * 
     * @param bookmarkId
    ...
    @GET
    @Produces({MediaType.APPLICATION_ATOM_XML, MediaType.APPLICATION_JSON})
    public SyndEntry getBookmark(@Context LinkBuilders linkBuilders, @Context UriInfo uriInfo,
        ...
        return createEntry(bookmarkId, bookmark, linkBuilders.createSystemLinksBuilder().subResource(bookmarkId), uriInfo);
    }

    /**
     * This method is invoked when the HTTP PUT method is issued by the client. This occurs only
     * when the requested representation (Http Accept header) is Atom (application/atom+xml) or Json
     * (application/json) and plain text is provided in the HTTP request message body of content
     * MIME type "text/plain" (header "Content-Type" must be "text/plain"). This method will update
     * the requested bookmark in the BookmarkStore with new content taken from the request message
     * body.
     * 
     * @param bookmarkId
    ...
    @Consumes(MediaType.TEXT_PLAIN)
    @Produces({MediaType.APPLICATION_ATOM_XML, MediaType.APPLICATION_JSON})
    public SyndEntry updateBookmark(String bookmark, @Context LinkBuilders linkBuilders, @Context UriInfo uriInfo,
            @PathParam("bookmark") String bookmarkId) {
    ...
    /**
     * This method is invoked when the HTTP DELETE method is issued by the client. This occurs only
     * when the requested representation (Http Accept header) is Atom (application/atom+xml) or Json
     * (application/json). This method deletes the bookmark from the BookmarkStore and returns the
     * deleted bookmark.
     * 
     * @param bookmarkId
     *            the bookmark id to update as it appears on the request uri
    ...
    @DELETE
    @Produces({MediaType.APPLICATION_ATOM_XML, MediaType.APPLICATION_JSON})
    public SyndEntry deleteBookmark(@Context LinkBuilders linkBuilders, @Context UriInfo uriInfo,
            @PathParam("bookmark") String bookmarkId) {
    ...
    private SyndEntry createEntry(String bookmarkId, String content, SystemLinksBuilder entryLinksBuilder, UriInfo uriInfo) {
