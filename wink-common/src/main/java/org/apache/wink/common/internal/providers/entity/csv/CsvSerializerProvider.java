...
public class CsvSerializerProvider implements MessageBodyWriter<CsvSerializer> {
    ...
    private final static HeaderDelegate<ContentDispositionHeader> header                     = RuntimeDelegate.getInstance().createHeaderDelegate(
                                                                                                 ContentDispositionHeader.class);


    public long getSize(CsvSerializer t, Class<?> type, Type genericType, Annotation[] annotations,
        MediaType mediaType) {
        return -1;
    }
    ...
    public void writeTo(CsvSerializer t, Class<?> type, Type genericType, Annotation[] annotations,
