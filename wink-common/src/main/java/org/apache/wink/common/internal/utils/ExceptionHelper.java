...
/**
 * Converts exception to a formatted string. If property <code>print.exception.stack.trace</code>
 * is set to true, the stack trace is returned, otherwise only exception name and message is returned.
 */
public final class ExceptionHelper {
    ...
    public static String toString(Throwable t) {
        ...
        if (s == null) s = "Exception " + t.getClass().getName() + " : " + t.getMessage();
        return s;
    }
        ...
        if (PRINT_STACK_TRACE) {
            StringWriter descr = new StringWriter() {
                ...
                public void write(char[] cbuf, int off, int len) {
                    if (canWrite()) super.write(cbuf, off, len);
                }

                public void write(int c) {
                    if (canWrite()) super.write(c);
                }

                public void write(String str, int off, int len) {
                    if (canWrite()) super.write(str, off, len);
                }

                public void write(String str) {
                    if (canWrite()) super.write(str);
                }
...
//        byte[] value;
//        try {
//            // Create "stream" containing single StackTraceElement with second instance prefix
...
//     */
//    private static StackTraceElement createStackTraceElement(String className, String methodName, String fileName, int lineNo) {
//        try {
...
//            // Now merge it with stackTraceElementPrefix and override second stream's magic with line number field value
//            byte[] ib = new byte[stackTraceElementStreamPrefix.length + db.length];
//            System.arraycopy(stackTraceElementStreamPrefix, 0, ib, 0, stackTraceElementStreamPrefix.length);
//            db[0] = (byte)((lineNo >> 24) & 0xff);
//            db[1] = (byte)((lineNo >> 16) & 0xff);
//            db[2] = (byte)((lineNo >> 8) & 0xff);
//            db[3] = (byte)(lineNo & 0xff);
//            System.arraycopy(db, 0, ib, stackTraceElementStreamPrefix.length, db.length);
//            // Finally read the elements - discard first (comes from prefix), use second one
//            ObjectInputStream ois = new ObjectInputStream(new ByteArrayInputStream(ib));
//            ois.readObject();
...
//        if (stackTraceElementStreamPrefix == null || s == null || s.length() == 0) return null;
//        StringTokenizer tokenizer = new StringTokenizer(s, "\n\r");
//        if (!tokenizer.hasMoreTokens()) return null;
//        return parseException(tokenizer.nextToken(), tokenizer, new LinkedList<Object>());
//    }
...
//     */
//    private static Throwable parseException(String firstLine, StringTokenizer tokenizer, List<Object> superStackTrace) {
//
//        // Format:
...
//                // Seems like "... nn more"
//                // make sure the stack trace doens't contain more than the specified number of elements
//                inMessage = false;
...
//        else exception = new ParsedException(message.substring(0, i), message.substring(i + 2));
