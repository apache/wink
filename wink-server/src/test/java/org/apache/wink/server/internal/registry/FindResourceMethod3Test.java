...
public class FindResourceMethod3Test extends MockServletInvocationTest {
            ...
            for(int i = 0; i < mediaTypes.size(); i++) {
                ...
                if (qualityString != null) {
                    ...
                    if (d.equals(Double.valueOf(0))) {
                        // intentionally returning the accept media type with q=0 to ensure outbound flow rejects it
                        ResponseBuilder responseBuilder = RuntimeDelegate.getInstance().createResponseBuilder();
                        // send back a media type that has q=0 -- this should be detected somewhere
                        responseBuilder.type(new MediaType("image", "jpeg"));
                        return responseBuilder.status(Response.Status.OK).entity("unknown string").build();
                    }
                }
            }
            return RuntimeDelegate.getInstance().createResponseBuilder().status(Response.Status.OK).entity("ResourceSimpleGetProduces.getAny").build();
        }
    ...
    public void testFindResourceSimple() throws Exception {
        ...
        request = MockRequestConstructor.constructMockRequest("GET", "/simpleGetProduces", "image/gif;q=0,image/jpeg;q=0.5");
        response = invoke(request);
        assertMethodFound(response, ResourceSimpleGetProduces.class, "getImageJpeg");
        
        // q=0 means do not return that media type
        request = MockRequestConstructor.constructMockRequest("GET", "/simpleGetProduces", "image/gif;q=0.5,image/jpeg;q=0");
        response = invoke(request);
        assertMethodFound(response, ResourceSimpleGetProduces.class, "getImageAny");
        
        // q=0 means do not return that media type
        request = MockRequestConstructor.constructMockRequest("GET", "/simpleGetProduces", "*/*,image/gif;q=0,image/jpeg;q=0,application/atom+xml;q=0");
        response = invoke(request);
        assertMethodFound(response, ResourceSimpleGetProduces.class, "getXml");
        
        // q=0 means do not return that media type
        request = MockRequestConstructor.constructMockRequest("GET", "/simpleGetProduces", "*/*,image/*;q=0,application/*;q=0");
        response = invoke(request);
        assertMethodFound(response, ResourceSimpleGetProduces.class, "getText");
        
        // q=0 means do not return that media type
        request = MockRequestConstructor.constructMockRequest("GET", "/simpleGetProduces", "text/plain,text/html;q=0");
        response = invoke(request);
        assertMethodFound(response, ResourceSimpleGetProduces.class, "getText");
        
        // q=0 means do not return that media type
        request = MockRequestConstructor.constructMockRequest("GET", "/simpleGetProduces", "text/plain;q=0,text/html");
        response = invoke(request);
        assertMethodFound(response, ResourceSimpleGetProduces.class, "getText");
        
        // q=0 means do not return that media type
        request = MockRequestConstructor.constructMockRequest("GET", "/simpleGetProduces", "*/*,text/plain;q=0,text/html;q=0,application/atom+xml;q=0");
        response = invoke(request);
        assertMethodFound(response, ResourceSimpleGetProduces.class, "getXml");
        
        // q=0 means do not return that media type.  In this test, we accept nothing.
        request = MockRequestConstructor.constructMockRequest("GET", "/simpleGetProduces", "*/*;q=0");
        response = invoke(request);
...
//        
//        // q=0 means do not return that media type
//        // since our method that supports * / * will try to return one of the excluded media types,
//        // PopulateResponseMediaTypeHandler should detect this and return a 406.
//        request = MockRequestConstructor.constructMockRequest("GET", "/simpleGetProduces", "*/*,image/*;q=0,application/*;q=0,text/*;q=0");
//        response = invoke(request);
//        assertMethodNotFound(response);
//        
//    }
    ...
    static void assertMethodNotFound(MockHttpServletResponse response) throws UnsupportedEncodingException {
