...
/**
 * Tests for case when it is unknown exactly, which data type (e.g. documentation pdf, work,
 * postscript, ...) of resource will be available.
 * <p/>
 * <em>Note: </em>Initiated by bug <tt>#39213</tt>.
 */
public class UnknownMimeTypeTest extends
    ...
    public static class UnknownMimeTypeResource {
        ...
        /**
         * Resource method for testing GET for unknown MIME type. The produced MIME type is unknown.
         * However the type of document can be requested in URI, as "get/pdf", meaning the requested
         * document is application/pdf. In the latter case header <tt>Content-Type</tt> is set in
         * response explicitly here.
         * 
         * @param typeName
         *            type of requested document as part of URI (get/pdf -> pdf document)
         * @param request
         *            SDK request
         * @return resource representation with content type set according to the actual resource
         *         MIME type
         */
        @GET
        ...
        public Response get(@PathParam("variable") String typeName, @Context HttpHeaders headers) {
            if (typeName.equals("unknown") || typeName.equals("atom")) {
                return RuntimeDelegate.getInstance().createResponseBuilder().status(Response.Status.OK).entity("unknown string")
                        .build();
                ...
                if (!accept.isAcceptable(MediaTypeUtils.PDF_TYPE)) {
                    return RuntimeDelegate.getInstance().createResponseBuilder().status(Response.Status.NOT_ACCEPTABLE).build();
                }
                return RuntimeDelegate.getInstance().createResponseBuilder().status(Response.Status.OK).entity("pdf string").header(
                        HttpHeaders.CONTENT_TYPE, MediaTypeUtils.PDF).build();
     ...
     * 
     * @throws IOException
     *             problem with getting resource for parsing
     * @throws XmlException
    ...
    /**
     * Test that if resource is pdf that content type in response is correctly set.
     * 
     * @throws IOException
     *             problem with getting resource for parsing
     * @throws XmlException
