...
public class InheritanceTest extends TestCase {
	...
	final private static String PARKING_LOT_URI = ServerEnvironmentInfo
			.getBaseURI()
			+ "/inheritance/parkinglot";

	final private static String PARKING_GARAGE_URI = ServerEnvironmentInfo
			.getBaseURI()
			+ "/inheritance/parkinggarage";

	final private static String CARPORT_URI = ServerEnvironmentInfo
			.getBaseURI()
			+ "/inheritance/carport";

	final private static String CARFERRY_URI = ServerEnvironmentInfo
			.getBaseURI()
			+ "/inheritance/carferry";

	final private static String CLASS_C_URI = ServerEnvironmentInfo
			.getBaseURI()
			+ "/inheritance/classc";

	final private static String FRUIT_URI = ServerEnvironmentInfo.getBaseURI()
			+ "/inheritance/fruit";
	...
	public void testSuperClassOverInterface() throws Exception {
		GetMethod getMethod = new GetMethod(CLASS_C_URI
				+ "/abstract_method1/encoded%20string");
		try {
			httpClient.executeMethod(getMethod);
			String resp = getMethod.getResponseBodyAsString();
	...
	public void testSubResourceLocaterInheritance() throws Exception {
		DeleteMethod deleteMethod = new DeleteMethod(PARKING_LOT_URI
				+ "/cars/remove/ParkingLot");
		...
		try {
			...
			deleteMethod = new DeleteMethod(PARKING_LOT_URI
					+ "/cars/remove/ParkingGarage");
			httpClient.executeMethod(deleteMethod);
			...
			assertEquals(
					"org.apache.wink.jaxrs.test.inheritance.fruits.Fruit;fruit%20suffix",
					response);
			getMethod.releaseConnection();
			...
			assertEquals(
					"org.apache.wink.jaxrs.test.inheritance.fruits.Apple;apple suffix", // parameters on class are not inherited
					response);
			getMethod.releaseConnection();
			...
			assertEquals(
					"org.apache.wink.jaxrs.test.inheritance.fruits.Orange;orange suffix",
					response);
